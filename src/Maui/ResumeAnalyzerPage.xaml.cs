...
public partial class ResumeAnalyzerPage : ContentPage
{
    ...
    {
        var result = GenerateAiPrompt();
        var copied = await CopyPromptToClipboard();
        result = result && copied && await OpenAiWebsite();
        return result;
    }

    private void OnGeneratePromptClicked(object? sender, EventArgs e) => GenerateAiPrompt();

    private bool GenerateAiPrompt()
    {
        ...
        try
        {
            ...
            ShowStatus("Prompt generated", Colors.Green);
        }
        catch (Exception ex)
        {
    ...
    private async void OnCopyPromptClicked(object? sender, EventArgs e) => await CopyPromptToClipboard();

    private async Task<bool> CopyPromptToClipboard()
    {
        try
        {
            if (string.IsNullOrWhiteSpace(AiPromptEditor.Text)) { ShowStatus("No prompt to copy", Colors.Orange); return false; }
        ...
        {
            ShowStatus($"Failed to copy: {ex.Message}", Colors.Red);
            return false;
        }
    }

    private async void OnOpenAiClicked(object? sender, EventArgs e) => await OpenAiWebsiteWithCopy();

    private async Task<bool> OpenAiWebsiteWithCopy()
    {
        await CopyPromptToClipboard();
        return await OpenAiWebsite();
    }

    private async Task<bool> OpenAiWebsite()
    {
        try
        {
